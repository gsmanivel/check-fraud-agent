...
Usage:
  ...
  python tests/eval/run_eval.py --engine both --report-dir tests/eval/reports
...
async def _run_one(engine: str, case: dict) -> dict:
    ...
    try:
        if engine == "sk":
            ...
            result = await run_agent_sk(payload, case_start)
        else:
...
def main():
    ...
    parser.add_argument("--engine", choices=["native", "sk", "both"], default="native",
    ...
    engines = ["native", "sk"] if args.engine == "both" else [args.engine]
    overall_min = 1.0
    all_reports = []

    for engine in engines:
        report = asyncio.run(run_eval(engine, args.cases, golden_path))
        all_reports.append(report)
        overall_min = min(overall_min, report["metrics"]["decision_accuracy"])
    ...
    bundle = all_reports[0] if len(all_reports) == 1 else {"runs": all_reports}
