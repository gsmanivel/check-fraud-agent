...
How it works:
  ...
  - Each check gets a fresh thread (no cross-check memory yet —
    threads keyed by account is a Phase 2 enhancement).
  - Tool calls execute client-side via enable_auto_function_calls;
    payload is passed to tool callables via a contextvar.

Switch engines: set TIER2_ENGINE=azure_agent
Requires:
  - AZURE_AI_AGENTS_ENDPOINT (falls back to AZURE_OPENAI_ENDPOINT)
  - DefaultAzureCredential (Entra ID — `az login` locally, Managed Identity in Azure)
"""
...
def _get_client() -> AgentsClient:
    ...
    if _client is None:
        endpoint = (
            os.environ.get("AZURE_AI_AGENTS_ENDPOINT")
...
def payee_verify(payee_name: str, amount: float = 0.0) -> str:
    ...
    suspicious = any(
        kw in payee_name.lower()
        for kw in ["cash", "bearer", "atm", "wire", "anonymous"]
    )
    return json.dumps({
        "payee_name":      payee_name,
        "suspicious":      suspicious,
        "risk_assessment": "high_risk" if suspicious else "low_risk",
    })
...
def _verdict(
    ...
    try:
        ...
        if start >= 0 and end > start:
            ...
            return FraudDecision(**data).model_dump()
